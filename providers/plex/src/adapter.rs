...
use trakkin_provider_sdk::{
    ...
    v1::{
        ...
        ReadTargetedStateResponse, RemoveSecret, ResolvePortableEndpointsRequest,
...
impl PlexAdapter {
    ...
    async fn open_server(
    ...
    ) -> Result<(PlexClient, ServerInfo, Vec<SecretPatch>), OperationFailure> {
        let client_identifier = settings
            .client_identifier
            .as_deref()
            .unwrap_or(&self.client_identifier);
        let client_identifier_patch = settings
            .client_identifier
            .is_none()
            .then(|| set_secret("client_identifier", client_identifier.as_bytes().to_vec()));
        if let Some(token) = &settings.token {
            let client =
                PlexClient::new(&settings.server_url, Some(token.clone()), client_identifier)
                    .map_err(|error| open_connection_failure(&error))?;
            match client.get_container::<ServerInfo>("").await {
                Ok(server) => {
                    return Ok((
                        client,
                        server,
                        client_identifier_patch.into_iter().collect(),
                    ));
                }
                Err(error)
                    if error.status() == Some(StatusCode::UNAUTHORIZED)
                        && settings.account_token.is_some() => {}
                Err(error) => return Err(open_connection_failure(&error)),
            }
        }

        let account_token = settings.account_token.clone().ok_or_else(|| {
            operation_failure(
        ...
        let identity_client = PlexClient::new(&settings.server_url, None, client_identifier)
            .map_err(|error| open_connection_failure(&error))?;
        let identity = identity_client
            .get_container::<ServerInfo>("/identity")
            .await
            .map_err(|error| {
                plex_failure(
                    &error,
                    "connection_identity_failed",
                    "The Plex server identity could not be read.",
                )
            })?;
        let cloud = PlexClient::new(&self.cloud_base_url, Some(account_token), client_identifier)
            .map_err(|error| open_connection_failure(&error))?;
        let resources = cloud
            .request_json::<Vec<Resource>>(
                Method::GET,
                "/api/v2/resources",
                &[
                    ("includeHttps", "1"),
            ...
            .map_err(|error| {
                plex_failure(
                    &error,
                    "connection_resource_discovery_failed",
        ...
        let resource = resources
            .into_iter()
            .find(|resource| {
                resource.client_identifier == identity.machine_identifier
                    && resource
                        .provides
                        .split(',')
                        .any(|capability| capability == "server")
        ...
        let token = resource
            ...
            .ok_or_else(|| {
                operation_failure(
                    OperationFailureCategory::Authorization,
                    "connection_server_access_denied",
                    "The signed-in Plex account does not have access to this server.",
                    false,
                )
            })?;
        let client = PlexClient::new(&settings.server_url, Some(token.clone()), client_identifier)
            .map_err(|error| open_connection_failure(&error))?;
        let server = client
            .get_container::<ServerInfo>("")
            .await
            .map_err(|error| open_connection_failure(&error))?;
        let mut secret_patches = vec![
            set_secret("token", token.into_bytes()),
            SecretPatch {
                key: "account_token".to_owned(),
                patch: Some(secret_patch::Patch::Remove(RemoveSecret {})),
            },
        ];
        secret_patches.extend(client_identifier_patch);
        Ok((client, server, secret_patches))
    }
...
impl AdapterService for PlexAdapter {
    ...
    async fn validate_connection(
        ...
        let field_problems = settings_from(&request.settings, &request.secrets)
            ...
            .unwrap_or_default()
            .into_iter()
            .filter(|problem| problem.path != "token" || problem.code != "required")
            .collect();
                        ...
                        Some(token) => {
                            ...
                            continue_authentication_response::Outcome::Result(
                                AuthenticationProgress {
                                    ...
                                    secret_patches: vec![
                                        set_secret("account_token", token.into_bytes()),
                                        set_secret(
                                            "client_identifier",
                                            self.client_identifier.as_bytes().to_vec(),
                                        ),
                                        SecretPatch {
                                            key: "token".to_owned(),
                                            patch: Some(secret_patch::Patch::Remove(
                                                RemoveSecret {},
                                            )),
                                        },
                                    ],
                ...
                OpenConnectionResult {
                    ...
                    capabilities: Some(ConnectionCapabilities {
                        reference_lookup: Some(LookupCapability {
                            reference_namespaces: mapping::REFERENCE_NAMESPACES
                                .map(str::to_owned)
                                .to_vec(),
    ...
    async fn lookup_portable_references(
        ...
        let request = request.into_inner();
        validation::lookup_request(&request).map_err(validation_status)?;
        if request.references.len() > MAX_LOOKUP_BATCH {
            return Err(Status::invalid_argument(
                "at most 50 references are allowed",
            ));
        }
        let connection = self.connection().await?;
        let section = Self::source_section(&connection, request.source_key.as_ref()).await?;
        ...
        for reference in &request.references {
            let outcome = if let Some(guid) = mapping::plex_guid(reference) {
...
fn settings_from(
    ...
    for secret in secrets {
        ...
        match secret.key.as_str() {
            "account_token" if !secret.value.is_empty() => {
                match String::from_utf8(secret.value.clone()) {
                    Ok(value) => account_token = Some(value),
                    Err(_) => problems.push(field_problem(
                        "account_token",
                        "invalid_type",
                        "Plex account token must be UTF-8 text.",
                    )),
                }
            }
            "account_token" => problems.push(field_problem(
                "account_token",
                "required",
                "Plex account token is required.",
            )),
            ...
            "client_identifier" if !secret.value.is_empty() => {
                match String::from_utf8(secret.value.clone()) {
                    Ok(value) => client_identifier = Some(value),
                    Err(_) => problems.push(field_problem(
                        "client_identifier",
                        "invalid_type",
                        "Plex client identifier must be UTF-8 text.",
                    )),
                }
            }
            "client_identifier" => problems.push(field_problem(
                "client_identifier",
                "required",
                "Plex client identifier is required.",
            )),
            _ => problems.push(field_problem(
                &secret.key,
                "unknown_field",
    ...
    if account_token.is_none()
        ...
        && !problems
            .iter()
            .any(|problem| matches!(problem.path.as_str(), "account_token" | "token"))
    {
        problems.push(field_problem(
            "token",
            "required",
...
fn set_secret(key: &str, value: Vec<u8>) -> SecretPatch {
    SecretPatch {
        key: key.to_owned(),
        patch: Some(secret_patch::Patch::Set(value)),
    }
}
...
fn open_connection_failure(error: &PlexError) -> OperationFailure {
    match error {
        ...
        PlexError::Response(response) if response.status() == Some(StatusCode::FORBIDDEN) => {
            operation_failure(
                OperationFailureCategory::Authorization,
    ...
    let (category, retryable) = match error {
        ...
        _ => match error.status() {
            ...
            Some(status) if status.is_server_error() => {
                (OperationFailureCategory::Unavailable, true)
            }
            Some(_) | None
                if matches!(error, PlexError::InvalidBaseUrl | PlexError::AssetTooLarge) =>
            {
